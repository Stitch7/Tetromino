...
public protocol GameDelegate {
    func display<ViewType>(piece: Piece<ViewType>)
    func remove<ViewType>(piece: Piece<ViewType>)
    func next<ViewType>(piece nextPiece: Piece<ViewType>)
...
public final class Game<ViewType: SquareViewType> {
    ...
    public func new(level: Level = .one) {
        board.reset()
        self.level = level
        score = 0
        gameOver = false
...
extension Game: UserInputDelegate {
    public func rotateLeft() {
        ...
        let rotatedPiece = currentPiece.rotatedLeft()
