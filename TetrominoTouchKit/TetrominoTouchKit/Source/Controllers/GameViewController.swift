...
final class GameViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        gameOverView.configureSubviews()
//        musicPlayer.play()
...
//        navigationItem.leftBarButtonItem = UIBarButtonItem(customView: levelView)
        navigationItem.leftBarButtonItem = UIBarButtonItem(barButtonSystemItem: .done, target: self, action: #selector(doneButtonPressed(sender:)))
...
extension GameViewController: GameDelegate {
    func gameOver() {
        ...
        if let viewsGestureRecognizers = view.gestureRecognizers {
            for gestureRecognizer in viewsGestureRecognizers {
        ...
        gameOverView.gameOverLabel.text = "GAME OVER"
    }
    ...
    func next<SquareView>(piece nextPiece: Piece<SquareView>) {
        nextPieceView.image = UIImage(named: nextPiece.type.rawValue, in: Bundle(for: GameViewController.self), compatibleWith: nil)!
    }
